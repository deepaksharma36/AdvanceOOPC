...
/**
 * this is a simple calculator which takes the input expression using digits and
 * operators in the precedence order and calculates the output.
 * 
...
public class Calculator {
	...
	public static float calculator_controller(String[] calcString) {
		...
		if (!calcString[calcString.length - 1].matches("(\\+)?(\\-)?\\d+(\\.\\d+)?")
			...
			System.out.println("Invalid Expression as last element is not digit " + "or closing parenthsis");
			System.exit(0);
			return 0;
		}
		...
		for (; indexCounter <= calcString.length - 1; indexCounter++) {
			...
			// numeric values from the input expression and pushes it into the stack.
			if (calcString[indexCounter].matches("(\\+)?(\\-)?\\d+(\\.\\d+)?")) {
				if ((lastWasOpe == true && lastWasDigit == false)) {
					...
					System.out.println("Invalid Expression as it starts from an operator");
					System.exit(0);
					return 0;
				}

			}
			// to extractvalid operators for non numerical values using
